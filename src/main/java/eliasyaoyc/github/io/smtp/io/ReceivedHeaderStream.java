...
public class ReceivedHeaderStream extends FilterInputStream {
  ...
  /**
   * @param softwareName A software name and version, or null if this information should not be
   *     printed
   * @param singleRecipient The single recipient of the message. If there are more than one
   *     recipients then this must be null.
   */
  public ReceivedHeaderStream(
      InputStream in,
      String heloHost,
      InetAddress host,
      String whoami,
  ...
  /**
   * Returns a formatted TCP-info element, depending on the success of the IP address name
   * resolution either with domain name or only the address literal.
   *
