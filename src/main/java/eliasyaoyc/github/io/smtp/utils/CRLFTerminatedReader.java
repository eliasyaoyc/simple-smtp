...
/**
 * {@link CRLFTerminatedReader} A Reader for use with SMTP or other protocols in which lines must
 * end with CRLF. Extends Reader and overrides its readLine() method. The Reader readLine() method
 * cannot serve for SMTP because it ends lines with either CR or LF alone.
 *
 * <p>JSS: The readline() method of this class has been 'enchanced' from the Apache JAMES version to
 * throw an IOException if the line is greater than or equal to MAX_LINE_LENGTH (998) which is
 * defined in <a href="http://rfc.net/rfc2822.html#s2.1.1.">RFC 2822</a>.
 *
...
public class CRLFTerminatedReader extends Reader {
   ...
   * @param charsetName the String name of a supported charset. "ASCII" is common here.
   ...
   *
   * @return either a String containing the contents of a line which must end with CRLF, or null if
   *     the end of the stream has been reached, possibly discarding some characters in a line not
      ...
      if (!cr_just_received) {
        ...
        switch (inChar) {
          ...
          case LF: // the normal ending of a line
            if (this.tainted == -1) this.tainted = this.lineBuffer.length();
